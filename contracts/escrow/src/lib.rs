...
mod types;
use types::{ContractStatus, Milestone};
...
impl Escrow {
    ...
    pub fn create_contract(
        ...
        // REDACTION POLICY: emit only addresses (public), contract id, and terms_hash (hash only).
        // Raw contract terms text, milestone descriptions, and any PII are never emitted.
        env.events().publish(
            (Symbol::new(&env, "contract_created"), id),
            (client, freelancer, arbiter, terms_hash),
        );
    ...
    pub fn deposit_funds(env: Env, contract_id: u32, amount: i128, caller: Address) -> bool {
        ...
        // REDACTION POLICY: emit only caller address and amount. No contract terms or PII.
        env.events().publish(
            (Symbol::new(&env, "funds_deposited"), contract_id),
            (caller, amount),
        );
    ...
    pub fn submit_milestone_work(
        ...
        if let Some(mut milestone) = milestones.get(milestone_index) {
            milestone.work_evidence = Some(work_evidence_hash.clone());
            milestones.set(milestone_index, milestone);
            contract.milestones = milestones.clone();
        } else {
        ...
        // REDACTION POLICY: emit only the hash and milestone index.
        // Raw work_evidence bytes (URLs, descriptions, documents) are NEVER emitted.
        env.events().publish(
            (Symbol::new(&env, "work_submitted"), contract_id),
            (milestone_index, work_evidence_hash),
        );

        true
    }

    pub fn approve_milestone(env: Env, contract_id: u32, milestone_index: u32, caller: Address) -> bool {
        caller.require_auth();
        ...
        // Only client or arbiter can approve
        ...
        // REDACTION POLICY: emit only milestone index and ledger timestamp. No participant data.
        env.events().publish(
            (Symbol::new(&env, "milestone_approved"), contract_id),
            (milestone_index, approval_time),
        );
    ...
    pub fn release_milestone(env: Env, contract_id: u32, milestone_index: u32, caller: Address) -> bool {
        ...
        // REDACTION POLICY: emit only milestone index and released amount. No participant data.
        env.events().publish(
            (Symbol::new(&env, "milestone_released"), contract_id),
            (milestone_index, released_amount),
        );

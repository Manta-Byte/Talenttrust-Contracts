...
fn create_contract(env: &Env, client: &EscrowClient) -> (Address, Address, u32) {
    ...
    let contract_id = client.create_contract(&client_addr, &freelancer_addr, &None, &milestone_amounts, &None, &None);
...
fn test_create_contract() {
    ...
    let id = client.create_contract(&client_addr, &freelancer_addr, &None, &milestone_amounts, &None, &None);
    assert_eq!(id, 0);
